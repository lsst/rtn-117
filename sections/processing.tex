...
During serial readout, a fraction of charge can be delayed rather than transferred with each clock cycle.
This deferred charge---charge transfer inefficiency (CTI)---produces serial charge tails and flux-dependent structure in overscan regions adjacent to the imaging area.
We measure and correct these effects with the extended pixel edge response (EPER) method of \citet{snyder2021}, implemented in calibration production (\texttt{cp\_pipe}) and instrument signature removal (\texttt{ip\_isr}).
...
Figure~\ref{fig:calib-dependency} shows that CTI production depends on defect masks and the linearizer; the PTC fit can in turn use CTI-corrected data when that correction is enabled upstream.
For each input exposure, the \texttt{cpCti} pipeline runs \texttt{cpCtiIsr} (\texttt{lsst.ip.isr.IsrTaskLSST}) with CTI statistics enabled, applying defect masking, linearity correction, and gains from the linearizer but not applying a CTI correction.
...
Following \citet{snyder2021}, the calibration parameterizes deferred charge at the amplifier level with a global serial CTI $b$ (\texttt{globalCti}), local electronic offsets with drift scale $A_L$ and decay time $\tau_L$ (\texttt{driftScale}, \texttt{decayTime}), and a serial charge-trap model (\texttt{serialTraps}) whose capture probability can depend on signal in the last imaging column.
